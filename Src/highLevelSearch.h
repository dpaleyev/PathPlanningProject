...
class HighLevelSearch {
    ...
    bool findCardinalConflict(TreeNode &node, std::vector<Constraint> &res, const Map &Map, const EnvironmentOptions &options);
    bool isCardinal(int id, int cost, int depth, const Map &Map, const EnvironmentOptions &options);
    bool findBypass(TreeNode &node, std::vector<Constraint> &conflict, ILogger *Logger, const Map &Map, const EnvironmentOptions &options);
public:
