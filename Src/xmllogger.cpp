...
/*void XmlLogger::writeToLogMap(const Map &map, const std::list<Node> &path)
{
    ...
    mapTag =
mapTag->FirstChildElement(CNS_TAG_LOG)->FirstChildElement(CNS_TAG_PATH);
    ...
    for (int i = 0; i < map.getMapHeight(); ++i) {
        ...
        for (int j = 0; j < map.getMapWidth(); ++j) {
            inPath = false;
            for(std::list<Node>::const_iterator it = path.begin(); it !=
path.end(); it++) if(it->i == i && it->j == j) { inPath = true; break;
                }
            if (!inPath)
...
/*void XmlLogger::writeToLogOpenClose(const typename &open, const typename
&close)
{
    //need to implement
    if (loglevel != CN_LP_LEVEL_FULL_WORD  && !(loglevel ==
CN_LP_LEVEL_MEDIUM_WORD && last)) return;
...
void XmlLogger::writeToLogPath(const std::vector<std::list<Node>> &path) {
  ...
  lplevel = lplevel->FirstChildElement(CNS_TAG_LOG)
                ->FirstChildElement(CNS_TAG_LPLEVEL);
  for (int i = 0; i < path.size(); ++i) {
    XMLElement *agent = doc.NewElement(CNS_TAG_AGENT);
    agent->SetAttribute(CNS_TAG_ATTR_NUM, i);
    int iterate = 0;
    for (std::list<Node>::const_iterator it = path[i].begin();
         it != path[i].end(); it++) {
...
void XmlLogger::writeToLogHPpath(const std::vector<std::list<Node>> &hppath) {
  ...
  hplevel = hplevel->FirstChildElement(CNS_TAG_LOG)
                ->FirstChildElement(CNS_TAG_HPLEVEL);
...
void XmlLogger::writeToLogSummary(float length, double time, double cellSize) {
  if (loglevel == CN_LP_LEVEL_NOPE_WORD)
    return;

  XMLElement *summary = doc.FirstChildElement(CNS_TAG_ROOT);
  summary =
      summary->FirstChildElement(CNS_TAG_LOG)->FirstChildElement(CNS_TAG_SUM);
