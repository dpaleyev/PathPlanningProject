...
SearchResult Search::startSearch(
    ILogger *Logger, const Map &map, const EnvironmentOptions &options,
    int agent_id,
    const std::unordered_map<
        int, std::unordered_set<std::pair<int, int>, pair_hash>> &constraints) {
  auto start_time = std::chrono::system_clock::now();
  sresult.pathfound = false;
  auto [start_i, start_j] = map.getStart(agent_id);
  Node *start =
      new Node{start_i, start_j,
               0,       getHeuristic(start_i, start_j, options, map, agent_id),
               0,       getHeuristic(start_i, start_j, options, map, agent_id),
               nullptr};
    ...
    for (int d_i = -1; d_i <= 1; d_i++) {
      for (int d_j = -1; d_j <= 1; d_j++) {
        if (constraints.find(s->step + 1) != constraints.end()) {
          if (constraints.at(s->step + 1).find({s->i + d_i, s->j + d_j}) !=
              constraints.at(s->step + 1).end()) {
            continue;
          }
        }
        ...
        if ((OPEN_find.find({s->i + d_i, s->j + d_j, s->step + 1}) ==
                 OPEN_find.end() &&
             CLOSED.find({s->i + d_i, s->j + d_j, s->step + 1}) ==
                 CLOSED.end())) {
          Node *s_new = new Node{
              s->i + d_i,
              s->j + d_j,
              s->step + 1,
              s->step + 1 +
                  getHeuristic(s->i + d_i, s->j + d_j, options, map, agent_id),
              s->g + d_dist,
              getHeuristic(s->i + d_i, s->j + d_j, options, map, agent_id),
              s};
        ...
        if (start->parent) {
          std::cout << start->parent << ' ' << start->parent->i << " "
                    << start->parent->j << '\n';
        }
      }
    }
...
double Search::getHeuristic(int i_cur, int j_cur,
