...
HighLevelResults HighLevelSearch::solve(ILogger *Logger, const Map &Map,
  ...
  while (!OPEN.empty()) {
    ...
    if (options.corridorsymmetry &&
        Map.CellIsIntoCorridor(conflict[0].cell.first,
                               conflict[0].cell.second)) {
    ...
    if (options.targetsymmetry && conflict[0].cell == conflict[1].cell) {
      for (int i = 0; i < 2; ++i) {
        if (conflict[0].cell == Map.getGoal(conflict[i].id)) {
          ...
          for (auto it = P.paths[conflict[1 - i].id]->begin();
               it != P.paths[conflict[1 - i].id]->end(); it++) {
          ...
          if (time < P.paths[conflict[i].id]->size()) {
            ...
            for (int d_time = 0; d_time <= 20; ++d_time) {
              constr.push_back(
                  {conflict[1 - i].id, time + d_time, conflict[i].cell});
            }
            TreeNode A(P, constr);
...
bool HighLevelSearch::findCardinalConflict(TreeNode &node,
  ...
  for (int i = 0; i < (int)node.paths.size(); i++) {
    for (int j = i + 1; j < (int)node.paths.size(); j++) {
      ...
      for (size_t k = 0;
           k < std::max(node.paths[i]->size(), node.paths[j]->size()); k++) {
        if (it1->i == it2->i && it1->j == it2->j) {
          if (isCardinal(i, node.costs[i], k, Map, options) ||
              isCardinal(j, node.costs[j], k, Map, options)) {
            res = {};
        ...
        if (k != 0) {
          ...
          if (it1->i == pr2->i && it1->j == pr2->j && pr1->i == it2->i &&
              pr1->j == it2->j) {
            if (isCardinal(i, node.costs[i], k, Map, options) ||
                isCardinal(j, node.costs[j], k, Map, options)) {
              res = {};
              ...
              flagCardinal = true;
              if (Map.CellIsIntoCorridor(res[0].cell.first,
                                         res[0].cell.second)) {
                return true;
              }
            }
...
bool HighLevelSearch::isCardinal(int id, int cost, int depth, const Map &Map,
                                 const EnvironmentOptions &options) {
  if (depth > cost) {
    return false;
  }
...
bool HighLevelSearch::findBypass(TreeNode &node,
                                 std::vector<Constraint> &conflict,
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    A.updatePath(Logger, Map, options, conflict[i].id);
    if (A.totalCost == node.totalCost &&
        A.countConflicts() < node.countConflicts()) {
      node.paths = A.paths;
      OPEN.push_back(node);
      return true;
    }
